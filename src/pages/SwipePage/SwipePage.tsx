...
export default function SwipePage() {
  ...
  useEffect(() => {
    if (profileIdQueue.length > 0 && currentIndexInBatch + 1 < profileIdQueue.length) {
      ...
      if (nextProfileId) {
        ...
        queries.profiles.get(nextProfileId).catch(err => {
  ...
  if (profileIdQueue.length === 0 && !isLoading) {
    ...
    return <div style={{ textAlign: 'center', padding: '20px' }}>{error || "Aucun profil disponible à swiper pour le moment."}</div>;
  }
  ...
  if (!currentProfileId && !isLoading) {
    ...
    // Cela pourrait indiquer la fin de tous les profils si une erreur est également définie.
     return <div style={{ textAlign: 'center', padding: '20px' }}>{error || "Aucun profil à afficher actuellement. Essayez de rafraîchir."}</div>;
  }
  ...
  return (
    <div style={{ textAlign: 'center', padding: '20px', maxWidth: '500px', margin: '0 auto' }}>
      ...
      {currentProfileId ? (
        <div key={currentProfileId}> {/* La clé est cruciale pour que FetchCard se re-rende avec de nouvelles props */}
